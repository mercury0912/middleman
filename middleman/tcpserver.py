...
class _TcpRelayHandler:
    ...
    def stop(self):
        ...
        gen_log.error("lread:%s lwrite: %s", self._streams.local.read_buffer_size, len(self._streams.local._write_buffer))
        gen_log.error("rread:%s rwrite: %s", self._streams.remote.read_buffer_size,
                      len(self._streams.remote._write_buffer))
        # close connection
        self._streams.close()
        self._streams = None
